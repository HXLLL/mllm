...
class FileDescriptor {
 public:
  ...
  FileDescriptor(FileDescriptor&& other) noexcept : path_(std::move(other.path_)), fd_(other.fd_) {
    other.fd_ = -1;
  }
  ...
  size_t read(void* buffer, size_t count) {
    ...
    if (ret < 0) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "Failed to read from file {}, errno: {} ({})",
                      path_.string(), errno, strerror(errno));
    }
    return ret;
  }
  size_t write(const void* buffer, size_t count) {
    ...
    if (ret < 0) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "Failed to write to file {}, errno: {} ({})",
                      path_.string(), errno, strerror(errno));
    }
    return ret;
  }
  ...
  void seek(off_t offset) { ::lseek(fd_, offset, SEEK_SET); }
...
static inline void fallocate_file(const fs::path& dir, const std::string& filename, size_t size) {
  ...
  if (fallocate(fd, 0, 0, size) != 0) {
    close(fd);
    MLLM_ERROR_EXIT(ExitCode::kIOError, "Failed to preallocate {} bytes for {}", size,
                    filepath.string());
  }
