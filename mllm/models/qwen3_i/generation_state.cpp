...
int GenerationState::writeHCacheLayer(FileDescriptor& file, int layer) {
...
int GenerationState::writeKVCacheLayer(FileDescriptor& file, const Tensor& cache, int layer) {
  auto [start_pos, count] =
      findWriteRange([&](int pos) { return layer_watermark_[pos] > layer && last_saved_watermark_[pos] <= layer; });
  if (count == 0) return 0;
...
void GenerationState::loadLayerKVCacheImpl(FileDescriptor& file, std::vector<Tensor>& cache, std::vector<uint8_t>& loaded,
...
void GenerationState::openFiles() {
  k_cache_file_ = FileDescriptor(path_ / "k_cache.bin");
  v_cache_file_ = FileDescriptor(path_ / "v_cache.bin");
  h_cache_file_ = FileDescriptor(path_ / "h_cache.bin");
  watermark_file_ = FileDescriptor(path_ / "layer_watermark.bin");
}
