#pragma once

/**
 * Direct I/O file abstraction with bounce buffer support.
 *
 * SyncFile:  Abstract base class - O_DIRECT, bounce buffer, sync interface
 * PosixFile: Uses standard pread/pwrite syscalls
 * AioFile:   Uses Linux AIO (io_submit/io_getevents)
 */

#include <cstddef>
#include <cstring>
#include <filesystem>

#include <fcntl.h>
#include <unistd.h>
#include <libaio.h>

#include "mllm/utils/Common.hpp"
#include "mllm/utils/Log.hpp"

namespace mllm::models::qwen3_i {

namespace fs = std::filesystem;

inline constexpr size_t kDirectIOAlignment = 4096;

inline size_t alignUp(size_t size, size_t alignment) {
  return (size + alignment - 1) & ~(alignment - 1);
}

// ============================================================================
// SyncFile: Abstract base class with O_DIRECT + bounce buffer
// ============================================================================

class SyncFile {
 public:
  SyncFile() = default;

  explicit SyncFile(const fs::path& path, size_t buffer_size = 1024 * 1024)
      : path_(path), buffer_size_(alignUp(buffer_size, kDirectIOAlignment)) {
    fd_ = ::open(path.c_str(), O_RDWR | O_CREAT | O_DIRECT, 0644);
    if (fd_ < 0) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "Failed to open file {} with O_DIRECT, errno: {} ({})",
                      path.string(), errno, strerror(errno));
    }

    int ret = posix_memalign(&buffer_, kDirectIOAlignment, buffer_size_);
    if (ret != 0) {
      ::close(fd_);
      MLLM_ERROR_EXIT(ExitCode::kIOError, "posix_memalign failed: {} ({})", ret, strerror(ret));
    }
  }

  virtual ~SyncFile() {
    if (fd_ >= 0) { ::close(fd_); }
    if (buffer_ != nullptr) { free(buffer_); }
  }

  SyncFile(const SyncFile&) = delete;
  SyncFile& operator=(const SyncFile&) = delete;

  SyncFile(SyncFile&& other) noexcept
      : path_(std::move(other.path_)),
        fd_(other.fd_),
        buffer_(other.buffer_),
        buffer_size_(other.buffer_size_) {
    other.fd_ = -1;
    other.buffer_ = nullptr;
  }

  SyncFile& operator=(SyncFile&& other) noexcept {
    if (this != &other) {
      if (fd_ >= 0) { ::close(fd_); }
      if (buffer_ != nullptr) { free(buffer_); }

      path_ = std::move(other.path_);
      fd_ = other.fd_;
      buffer_ = other.buffer_;
      buffer_size_ = other.buffer_size_;

      other.fd_ = -1;
      other.buffer_ = nullptr;
    }
    return *this;
  }

  virtual ssize_t pread(void* buf, size_t count, off_t offset) = 0;
  virtual ssize_t pwrite(const void* buf, size_t count, off_t offset) = 0;

  void fsync() { ::fsync(fd_); }

  [[nodiscard]] int fd() const { return fd_; }
  [[nodiscard]] const fs::path& path() const { return path_; }

 protected:
  // Calculate aligned parameters: (aligned_offset, prefix_skip, aligned_count)
  std::tuple<off_t, size_t, size_t> alignParams(off_t offset, size_t count) const {
    off_t aligned_offset = offset & ~static_cast<off_t>(kDirectIOAlignment - 1);
    size_t prefix_skip = offset - aligned_offset;
    size_t aligned_count = alignUp(prefix_skip + count, kDirectIOAlignment);
    MLLM_RT_ASSERT(aligned_count <= buffer_size_);
    return {aligned_offset, prefix_skip, aligned_count};
  }

  fs::path path_;
  int fd_ = -1;
  void* buffer_ = nullptr;
  size_t buffer_size_ = 0;
};

// ============================================================================
// PosixFile: pread/pwrite syscalls with O_DIRECT
// ============================================================================

class PosixFile : public SyncFile {
 public:
  PosixFile() = default;
  explicit PosixFile(const fs::path& path, size_t buffer_size = 1024 * 1024)
      : SyncFile(path, buffer_size) {}

  ssize_t pread(void* buf, size_t count, off_t offset) override {
    auto [aligned_offset, prefix_skip, aligned_count] = alignParams(offset, count);

    ssize_t ret = ::pread(fd_, buffer_, aligned_count, aligned_offset);
    if (ret < 0) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "pread failed on {}, errno: {} ({})", path_.string(),
                      errno, strerror(errno));
    }

    memcpy(buf, static_cast<char*>(buffer_) + prefix_skip, count);
    return static_cast<ssize_t>(count);
  }

  ssize_t pwrite(const void* buf, size_t count, off_t offset) override {
    auto [aligned_offset, prefix_skip, aligned_count] = alignParams(offset, count);

    // Read-modify-write if not aligned
    if (prefix_skip > 0 || (prefix_skip + count) % kDirectIOAlignment != 0) {
      ::pread(fd_, buffer_, aligned_count, aligned_offset);
    }

    memcpy(static_cast<char*>(buffer_) + prefix_skip, buf, count);

    ssize_t ret = ::pwrite(fd_, buffer_, aligned_count, aligned_offset);
    if (ret < 0) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "pwrite failed on {}, errno: {} ({})", path_.string(),
                      errno, strerror(errno));
    }

    return static_cast<ssize_t>(count);
  }
};

// ============================================================================
// AioFile: Linux AIO (io_submit/io_getevents)
// ============================================================================

class AioContext {
 public:
  explicit AioContext(int max_events = 64) : ctx_(nullptr) {
    int ret = io_setup(max_events, &ctx_);
    if (ret < 0) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "io_setup failed: {} ({})", -ret, strerror(-ret));
    }
  }

  ~AioContext() {
    if (ctx_ != nullptr) { io_destroy(ctx_); }
  }

  AioContext(const AioContext&) = delete;
  AioContext& operator=(const AioContext&) = delete;

  AioContext(AioContext&& other) noexcept : ctx_(other.ctx_) { other.ctx_ = nullptr; }

  AioContext& operator=(AioContext&& other) noexcept {
    if (this != &other) {
      if (ctx_ != nullptr) { io_destroy(ctx_); }
      ctx_ = other.ctx_;
      other.ctx_ = nullptr;
    }
    return *this;
  }

  [[nodiscard]] io_context_t get() const { return ctx_; }

 private:
  io_context_t ctx_{};
};

class AioFile : public SyncFile {
 public:
  AioFile() = default;
  explicit AioFile(const fs::path& path, size_t buffer_size = 1024 * 1024)
      : SyncFile(path, buffer_size) {}

  ssize_t pread(void* buf, size_t count, off_t offset) override {
    auto [aligned_offset, prefix_skip, aligned_count] = alignParams(offset, count);

    struct iocb cb;
    struct iocb* cbs[1] = {&cb};
    io_prep_pread(&cb, fd_, buffer_, aligned_count, aligned_offset);

    int ret = io_submit(ctx_.get(), 1, cbs);
    if (ret != 1) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "io_submit pread failed: {} ({})", -ret, strerror(-ret));
    }

    struct io_event event;
    ret = io_getevents(ctx_.get(), 1, 1, &event, nullptr);
    if (ret != 1) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "io_getevents failed: {} ({})", -ret, strerror(-ret));
    }

    if (static_cast<ssize_t>(event.res) < 0) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "AIO pread failed: {} ({})", -event.res,
                      strerror(-event.res));
    }

    memcpy(buf, static_cast<char*>(buffer_) + prefix_skip, count);
    return static_cast<ssize_t>(count);
  }

  ssize_t pwrite(const void* buf, size_t count, off_t offset) override {
    auto [aligned_offset, prefix_skip, aligned_count] = alignParams(offset, count);

    // Read-modify-write if not aligned
    if (prefix_skip > 0 || (prefix_skip + count) % kDirectIOAlignment != 0) {
      struct iocb cb;
      struct iocb* cbs[1] = {&cb};
      io_prep_pread(&cb, fd_, buffer_, aligned_count, aligned_offset);

      if (io_submit(ctx_.get(), 1, cbs) == 1) {
        struct io_event event;
        io_getevents(ctx_.get(), 1, 1, &event, nullptr);
      }
    }

    memcpy(static_cast<char*>(buffer_) + prefix_skip, buf, count);

    struct iocb cb;
    struct iocb* cbs[1] = {&cb};
    io_prep_pwrite(&cb, fd_, buffer_, aligned_count, aligned_offset);

    int ret = io_submit(ctx_.get(), 1, cbs);
    if (ret != 1) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "io_submit pwrite failed: {} ({})", -ret, strerror(-ret));
    }

    struct io_event event;
    ret = io_getevents(ctx_.get(), 1, 1, &event, nullptr);
    if (ret != 1) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "io_getevents failed: {} ({})", -ret, strerror(-ret));
    }

    if (static_cast<ssize_t>(event.res) < 0) {
      MLLM_ERROR_EXIT(ExitCode::kIOError, "AIO pwrite failed: {} ({})", -event.res,
                      strerror(-event.res));
    }

    return static_cast<ssize_t>(count);
  }

 private:
  AioContext ctx_;
};

// ============================================================================
// Default type alias and utilities
// ============================================================================

using File = AioFile;

inline void preallocate_file(const fs::path& dir, const std::string& filename, size_t size) {
  if (!fs::exists(dir)) { fs::create_directories(dir); }

  auto filepath = dir / filename;
  size_t aligned_size = alignUp(size, kDirectIOAlignment);

  int fd = open(filepath.c_str(), O_CREAT | O_WRONLY, 0644);
  if (fd < 0) {
    MLLM_ERROR_EXIT(ExitCode::kIOError, "Failed to create file {}", filepath.string());
  }

  if (fallocate(fd, 0, 0, aligned_size) != 0) {
    close(fd);
    MLLM_ERROR_EXIT(ExitCode::kIOError, "Failed to preallocate {} bytes for {}", aligned_size,
                    filepath.string());
  }
  close(fd);
}

}  // namespace mllm::models::qwen3_i
